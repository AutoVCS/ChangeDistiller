...
public class WhenMethodBodiesAreConverted extends WhenASTsAreConverted {
    ...
    public void forStatementWithConditionShouldBeConverted () throws Exception {
        ...
        assertThat( getTreeString(), is(
                "method { (i < list.size()) { System.out.print('.');,int i = 0; { int i = 0; },i ++ { i ++; } } }" ) );
        assertSourceRangeCorrectness( getFirstChild() );
    }
    ...
    public void ifStatementShouldBeConverted () throws Exception {
        ...
        assertThat( getTreeString(), is(
                "method { list.isEmpty() { list.isEmpty() { System.out.print(\"empty\"); },list.isEmpty() { System.out.print(\"not empty\"); } } }" ) );
        assertSourceRangeCorrectness( getFirstChild() );
    }
    ...
    public void ifStatementWithoutElseShouldBeConverted () throws Exception {
        ...
        assertThat( getTreeString(),
                is( "method { list.isEmpty() { list.isEmpty() { System.out.print(\"empty\"); } } }" ) );
        assertSourceRangeCorrectness( getFirstChild() );
    }
    ...
    public void returnStatementShouldBeConverted () throws Exception {
        ...
        convert();
        assertThat( getFirstLeaf().getLabel(), is( JavaEntityType.RETURN_STATEMENT ) );
        assertThat( getTreeString(), is( "method { return Math.min(a, b); }" ) );
        assertSourceRangeCorrectness();
    }
    ...
    public void switchStatementShouldBeConverted () throws Exception {
        ...
        assertThat( getTreeString(),
                is( "method { foo { [ONE],a = 1;,,default,a = 2;,[ONE],a = 1;,,default,a = 2; } }" ) );
        assertSourceRangeCorrectness( getFirstChild() );
    }
    ...
    public void tryStatementShouldBeConverted () throws Exception {
        fSnippet = "try { foo.bar(e); } catch (IOException e) { return 2; } catch (Exception e) { return 3; } finally { cleanup(); }";
        prepareCompilation();
        convert();
        assertThat( getFirstChild().getLabel(), is( JavaEntityType.TRY_STATEMENT ) );
        assertThat( ( (Node) ( (Node) getFirstChild().getFirstChild() ).getNextSibling() ).getLabel(),
                is( JavaEntityType.CATCH_CLAUSES ) );
        assertThat( ( (Node) ( (Node) getFirstChild().getFirstChild() ).getNextSibling().getFirstChild() ).getLabel(),
                is( JavaEntityType.CATCH_CLAUSE ) );
        assertThat( ( (Node) getFirstChild().getLastChild() ).getLabel(), is( JavaEntityType.FINALLY ) );
        assertThat( getTreeString(), is(
                "method {  {  { foo.bar(e); }, { IOException { return 2; },Exception { return 3; } }, { cleanup(); } } }" ) );
        assertSourceRangeCorrectness( getFirstChild() );
    }
    ...
    public void tryStatementWithoutFinallyShouldBeConverted () throws Exception {
        ...
        assertThat( ( (Node) ( (Node) getFirstChild().getFirstChild() ).getNextSibling().getFirstChild() ).getLabel(),
                is( JavaEntityType.CATCH_CLAUSE ) );
        assertThat( getTreeString(),
                is( "method {  {  { foo.bar(e); }, { IOException { return 2; },Exception { return 3; } } } }" ) );
        assertSourceRangeCorrectness( getFirstChild() );
    }
    ...
    private void assertSourceRangeCorrectness ( final Node node ) {
        final SourceCodeEntity entity = node.getEntity();
        final String source = fCompilation.getSource().substring( entity.getStartPosition(),
                entity.getEndPosition() + 1 );
    ...
    private void convert () {
        ...
        final AbstractMethodDeclaration method = CompilationUtils.findMethod( fCompilation.getCompilationUnit(),
                "method" );
    ...
    @Override
    protected String getSourceCodeWithSnippets ( final String... snippets ) {
        final StringBuilder src = new StringBuilder( "public class Foo { " );
        src.append( "public void method() { " );
        for ( final String statement : snippets ) {
