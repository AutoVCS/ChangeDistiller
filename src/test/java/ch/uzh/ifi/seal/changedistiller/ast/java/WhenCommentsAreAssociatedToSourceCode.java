...
public class WhenCommentsAreAssociatedToSourceCode extends JavaDistillerTestCase {
    ...
    public static void prepareCompilationUnit () throws Exception {
        ...
        final List<Comment> comments = CompilationUtils.extractComments( sCompilation );
        final CommentCleaner visitor = new CommentCleaner( sCompilation.getSource() );
        ...
        final AbstractMethodDeclaration method = CompilationUtils.findMethod( sCompilation.getCompilationUnit(),
                "foo" );
        final JavaMethodBodyConverter bodyT = sInjector.getInstance( JavaMethodBodyConverter.class );
    ...
    public void proximityRatingShouldAssociateCommentToClosestEntity () throws Exception {
        final Node node = findNode( "boolean check = (number > 0);" );
    ...
    public void undecidedProximityRatingShouldAssociateCommentToNextEntity () throws Exception {
        final Node node = findNode( "check" );
        assertCorrectAssociation( node, "// check the interesting number" + LF + "        // and some new else",
    ...
    public void commentInsideSimpleStatementShouldBeAssociatedToThatStatement () throws Exception {
        final Node node = findNode( "b = Math.round(Math.random());" );
        assertCorrectAssociation( node, "/* inner comment */", JavaEntityType.BLOCK_COMMENT );
    }

    private void assertCorrectAssociation ( final Node node, final String expectedComment,
            final EntityType expectedCommentType ) {
        final List<Node> associatedNodes = node.getAssociatedNodes();
    ...
    private Node findNode ( final String value ) {
        for ( final Enumeration<Node> e = (Enumeration<Node>) (Enumeration< ? >) sRoot.breadthFirstEnumeration(); e
                .hasMoreElements(); ) {
