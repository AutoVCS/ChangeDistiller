 ...
 *
 * <p>
 * {@link TreeDifferencer} can only apply the matching and edit script
 * generation if the two trees are made out of such nodes.
 *
 * @author Beat Fluri
 * @see TreeDifferencer
 */
public class Node extends DefaultMutableTreeNode implements TreeNode {
    ...
    private final List<Node>  fAssociatedNodes = new ArrayList<Node>();
     ...
     *            of the node
     */
    public Node ( final EntityType label, final String value ) {
        super();
        fLabel = label;
        fValue = value;
    }
     ...
     *
     * @return <code>true</code> if this node is match with another node,
     ...
     *
     * @return <code>true</code> if this node is in order with its siblings,
     *         <code>false</code> otherwise
     */
    ...
    public void setLabel ( final EntityType label ) {
        fLabel = label;
    }
     ...
     */
    public void addAssociatedNode ( final Node node ) {
    ...
    public void setEntity ( final SourceCodeEntity entity ) {
        fEntity = entity;
    }

    /**
     * Prints this {@link Node} and its children with
     * <code>value ['{' child [, child]* '}']</code>.
     *
     * @param output
    ...
    public StringBuilder print ( final StringBuilder output ) {
        output.append( getValue() );
        if ( !isLeaf() ) {
            output.append( " { " );
            for ( final Iterator<Node> it = (Iterator<Node>) (Iterator< ? >) children.iterator(); it.hasNext(); ) {
    ...
    // Removing hashCode and equals fixes issues #1, #8, #9. See
    // https://bitbucket.org/sealuzh/tools-changedistiller/issue/1.
    ...
    /**
     * Returns true if the leaf candidate is a leaf and is a descendant of this
     * node.
     *
     * @param candidate
     *            to check for leaf descendant
     * @return <code>true</code> if the candidate is a leaf and a descendant of
     *         this node, <code>false</code> otherwise
     */
    public boolean isLeafDescendant ( final Node candidate ) {
