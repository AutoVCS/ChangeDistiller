...
public class JavaASTHelper implements ASTHelper<JavaStructureNode> {
    ...
    private final JavaDeclarationConverter fDeclarationConverter;
    private final JavaMethodBodyConverter  fBodyConverter;
    private final JavaASTNodeTypeConverter fASTHelper;
    private final JavaCompilation          fCompilation;
    private List<Comment>                  fComments;

    @Inject
    JavaASTHelper ( @Assisted final File file, @Assisted final String javaVersion,
            final JavaASTNodeTypeConverter astHelper, final JavaDeclarationConverter declarationConverter,
            final JavaMethodBodyConverter bodyConverter ) {

        fCompilation = JavaCompilationUtils.compile( file, getVersionNumber( javaVersion ) );
        prepareComments();
    ...
    private long getVersionNumber ( final String javaVersion ) {
        long versionNumber;
        switch ( javaVersion ) {
            case "1.1":
            ...
            case "9":
                ...
                break;
            case "10":
                versionNumber = ClassFileConstants.JDK10;
                break;
            case "11":
                versionNumber = ClassFileConstants.JDK11;
                break;
            case "12":
                versionNumber = ClassFileConstants.JDK12;
                break;
            case "13":
                versionNumber = ClassFileConstants.JDK13;
                break;
            case "14":
                versionNumber = ClassFileConstants.JDK14;
                break;
            case "15":
                versionNumber = ClassFileConstants.JDK15;
                break;
            case "16":
                versionNumber = ClassFileConstants.JDK16;
                break;
            default:
                versionNumber = ClassFileConstants.JDK16;
        }
    ...
    private void cleanComments ( final List<Comment> comments ) {
        final CommentCleaner visitor = new CommentCleaner( fCompilation.getSource() );
    ...
    private List<Comment> collectComments () {
        final CommentCollector collector = new CommentCollector( fCompilation.getCompilationUnit(),
    ...
    @Override
    public Node createDeclarationTree ( final JavaStructureNode node ) {
        final ASTNode astNode = node.getASTNode();
        final Node root = createRootNode( node, astNode );
        return createDeclarationTree( astNode, root );
    }

    private Node createDeclarationTree ( final ASTNode astNode, final Node root ) {
    ...
    @Override
    public Node createDeclarationTree ( final JavaStructureNode node, final String qualifiedName ) {
        final ASTNode astNode = node.getASTNode();
        final Node root = createRootNode( node, astNode );
    ...
    private Node createRootNode ( final JavaStructureNode node, final ASTNode astNode ) {
        final Node root = new Node( fASTHelper.convertNode( astNode ), node.getFullyQualifiedName() );
        root.setEntity( createSourceCodeEntity( node ) );
        return root;
    }

    @Override
    public Node createMethodBodyTree ( final JavaStructureNode node ) {
        final ASTNode astNode = node.getASTNode();
    ...
    public JavaStructureNode createStructureTree () {
        final CompilationUnitDeclaration cu = fCompilation.getCompilationUnit();
        final JavaStructureNode node = new JavaStructureNode( Type.CU, null, null, cu );
        cu.traverse( new JavaStructureTreeBuilder( node ), (CompilationUnitScope) null );
        return node;
    }

    @Override
    public EntityType convertType ( final JavaStructureNode node ) {
        return fASTHelper.convertNode( node.getASTNode() );
    }

    @Override
    public SourceCodeEntity createSourceCodeEntity ( final JavaStructureNode node ) {
        return new SourceCodeEntity( node.getFullyQualifiedName(), convertType( node ),
                extractModifier( node.getASTNode() ), createSourceRange( node.getASTNode() ) );
    }

    private SourceRange createSourceRange ( final ASTNode astNode ) {
        if ( astNode instanceof TypeDeclaration ) {
            final TypeDeclaration type = (TypeDeclaration) astNode;
            return new SourceRange( type.declarationSourceStart, type.declarationSourceEnd );
        }
        if ( astNode instanceof AbstractMethodDeclaration ) {
            final AbstractMethodDeclaration method = (AbstractMethodDeclaration) astNode;
            return new SourceRange( method.declarationSourceStart, method.declarationSourceEnd );
        }
        if ( astNode instanceof FieldDeclaration ) {
            final FieldDeclaration field = (FieldDeclaration) astNode;
    ...
    @Override
    public StructureEntityVersion createStructureEntityVersion ( final JavaStructureNode node,
            final String versionNum ) {
        return new StructureEntityVersion( convertType( node ), node.getFullyQualifiedName(),
    ...
    private int extractModifier ( final ASTNode node ) {
        int ecjModifer = -1;
    ...
    private int convertECJModifier ( final int ecjModifer ) {
        int modifier = 0x0;
    ...
    private boolean isNative ( final int ecjModifier ) {
        return ( ecjModifier & ClassFileConstants.AccNative ) != 0;
    }

    private boolean isStatic ( final int ecjModifier ) {
        return ( ecjModifier & ClassFileConstants.AccStatic ) != 0;
    }

    private boolean isStrictFP ( final int ecjModifier ) {
        return ( ecjModifier & ClassFileConstants.AccStrictfp ) != 0;
    }

    private boolean isSynchronized ( final int ecjModifier ) {
        return ( ecjModifier & ClassFileConstants.AccSynchronized ) != 0;
    }

    private boolean isTransient ( final int ecjModifier ) {
        return ( ecjModifier & ClassFileConstants.AccTransient ) != 0;
    }

    private boolean isVolatile ( final int ecjModifier ) {
        return ( ecjModifier & ClassFileConstants.AccVolatile ) != 0;
    }

    private boolean isAbstract ( final int ecjModifier ) {
        return ( ecjModifier & ClassFileConstants.AccAbstract ) != 0;
    }

    private boolean isPrivate ( final int ecjModifier ) {
        return ( ecjModifier & ClassFileConstants.AccPrivate ) != 0;
    }

    private boolean isProtected ( final int ecjModifier ) {
        return ( ecjModifier & ClassFileConstants.AccProtected ) != 0;
    }

    private boolean isPublic ( final int ecjModifier ) {
        return ( ecjModifier & ClassFileConstants.AccPublic ) != 0;
    }

    private boolean isFinal ( final int ecjModifier ) {
        return ( ecjModifier & ClassFileConstants.AccFinal ) != 0;
    }

    @Override
    public StructureEntityVersion createMethodInClassHistory ( final ClassHistory classHistory,
            final JavaStructureNode node, final String versionNum ) {
        MethodHistory mh;
        final StructureEntityVersion method = createStructureEntityVersion( node, versionNum );
    ...
    @Override
    public StructureEntityVersion createMethodInClassHistory ( final ClassHistory classHistory,
            final JavaStructureNode node ) {
        MethodHistory mh;
        final StructureEntityVersion method = createStructureEntityVersion( node );
        if ( classHistory.getMethodHistories().containsKey( method.getUniqueName() ) ) {
            mh = classHistory.getMethodHistories().get( method.getUniqueName() );
            mh.addVersion( method );
        }
    ...
    @Override
    public StructureEntityVersion createFieldInClassHistory ( final ClassHistory classHistory,
            final JavaStructureNode node, final String versionNum ) {
        AttributeHistory ah = null;
        final StructureEntityVersion attribute = createStructureEntityVersion( node, versionNum );
    ...
    public StructureEntityVersion createFieldInClassHistory ( final ClassHistory classHistory,
            final JavaStructureNode node ) {
        AttributeHistory ah = null;
        final StructureEntityVersion attribute = createStructureEntityVersion( node );
        if ( classHistory.getAttributeHistories().containsKey( attribute.getUniqueName() ) ) {
            ah = classHistory.getAttributeHistories().get( attribute.getUniqueName() );
            ah.addVersion( attribute );
        }
    ...
    @Override
    public StructureEntityVersion createInnerClassInClassHistory ( final ClassHistory classHistory,
            final JavaStructureNode node, final String versionNum ) {
        ClassHistory ch = null;
        final StructureEntityVersion clazz = createStructureEntityVersion( node, versionNum );
    ...
    public StructureEntityVersion createInnerClassInClassHistory ( final ClassHistory classHistory,
            final JavaStructureNode node ) {
        ClassHistory ch = null;
        final StructureEntityVersion clazz = createStructureEntityVersion( node );
        if ( classHistory.getInnerClassHistories().containsKey( clazz.getUniqueName() ) ) {
            ch = classHistory.getInnerClassHistories().get( clazz.getUniqueName() );
            ch.addVersion( clazz );
        }
